...
async function rejectSymlinkPath(filePath: string, relPath: string): Promise<ReturnType<typeof errorResult> | undefined> {
  const linkStat = await lstat(filePath);
  if (linkStat.isSymbolicLink()) {
    return errorResult('symlink_path', `Refusing to write directly to symlink path: ${relPath}`, { path: relPath });
  }
  return undefined;
}
    ...
    execute: async (toolCallId: string, params: unknown) => {
      ...
      try {
        ...
        try {
          const linkStat = await lstat(filePath);
          if (linkStat.isSymbolicLink()) {
            return errorResult('symlink_path', `Refusing to write directly to symlink path: ${toWorkspaceRelative(cwd, filePath)}`, {
              path: toWorkspaceRelative(cwd, filePath),
            });
          }
          if (!linkStat.isFile()) {
  ...
  return {
    ...
    execute: async (toolCallId: string, params: unknown) => {
        ...
        try {
          ...
          for (const operation of operations) {
            if (operation.type === 'add') {
              ...
              let originalContent: string | null = null;
              try {
                const symlinkError = await rejectSymlinkPath(filePath, relPath);
                if (symlinkError) {
                  const failure = { ...symlinkError.details, type: 'create' };
                  if (perFileResults.length === 0) return symlinkError;
                  perFileResults.push(failure);
                  break;
                }
      ...
      try {
        ...
        const symlinkError = await rejectSymlinkPath(filePath, relPathForChecks);
