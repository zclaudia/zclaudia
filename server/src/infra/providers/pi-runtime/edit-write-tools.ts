...
import { applyLineEndingStyle, buildContentDetailFields, lineEndingFor, maybeMarkExecutableForShebang, readTextFileWithMetadata, writeTextFileAtomic } from './text-io.js';
...
function capStructuredPatchLines(hunks: FileDiffHunk[]): { structuredPatch: FileDiffHunk[]; truncated: boolean } {
...
async function resolveWritePath(
  ...
  filePath: string,
  relPath: string,
): Promise<{ ok: true; writePath: string; wasSymlink: boolean } | { ok: false; result: ReturnType<typeof errorResult> }> {
  let linkStat;
  try {
    linkStat = await lstat(filePath);
  } catch (err) {
    if ((err as NodeJS.ErrnoException)?.code === 'ENOENT') return { ok: true, writePath: filePath, wasSymlink: false };
    throw err;
  }
  ...
  try {
    ...
    return { ok: false, result: errorResult('broken_symlink', `Refusing to write through broken symlink: ${relPath}`, { path: relPath }) };
  }
  ...
  const insideWorkspace = relToWorkspace === '' || (!relToWorkspace.startsWith('..') && !path.isAbsolute(relToWorkspace));
  if (!insideWorkspace) {
    return { ok: false, result: errorResult('symlink_escape', `Refusing to write through symlink whose target is outside the workspace: ${relPath}`, { path: relPath }) };
  }
...
export function createWriteBridgeTool(cwd: string, options?: FileMutationToolOptions): AgentTool<any> {
  return {
    ...
    execute: async (toolCallId: string, params: unknown) => {
      ...
      if (contentGuard) {
        return errorResult(contentGuard.code, contentGuard.message, guardErrorDetails(contentGuard, {
      ...
      try {
        ...
        const relPath = toWorkspaceRelative(cwd, filePath);
        const backup = originalContent !== null ? await recordFileBackup(relPath, originalContent, writePath) : undefined;
  ...
  return {
    ...
    execute: async (toolCallId: string, params: unknown) => {
        ...
        try {
          ...
          for (const operation of operations) {
            if (operation.type === 'add') {
              ...
              if (contentGuard) {
                const failure = { ok: false, error: contentGuard.code, message: contentGuard.message, path: relPath, type: 'create', ...(contentGuard.details ?? {}) };
                if (perFileResults.length === 0) return textResult(failure.message, failure);
                perFileResults.push(failure);
                break;
              }
      ...
      if (replacementGuard?.guard) {
        return errorResult(replacementGuard.guard.code, replacementGuard.guard.message, guardErrorDetails(replacementGuard.guard, {
          path: toWorkspaceRelative(cwd, filePath),
      ...
      try {
        ...
        if (fileStat.size > MAX_EDIT_FILE_BYTES) {
          return errorResult('file_too_large', `File is too large to edit safely: ${relPathForChecks}`, {
        ...
        if (batchEdits) {
          ...
          await writeTextFileAtomic(writePath, applyLineEndingStyle(updated, lineEndingFor(original)), originalMetadata);
