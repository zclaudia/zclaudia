...
import {
  createEditBridgeTool as createFileEditBridgeTool,
  createMultiEditBridgeTool,
  createWriteBridgeTool as createFileWriteBridgeTool,
} from './edit-write-tools.js';
...
export const BUILTIN_TOOL_FACTORIES: Record<ToolName, ToolFactory> = {
  ...
  TaskOutput: (_cwd, options) =>
    createTaskOutputTool(options?.db, options?.taskRuntimeRegistryFactory),
  Monitor: (_cwd, options) =>
    createMonitorTool(
      options?.sessionId,
      options?.runId,
      options?.db,
      options?.taskRuntimeRegistryFactory
    ),
  Agent: (cwd, options) =>
    createAgentTool(
      cwd,
