...
export function createEnterWorktreeTool(cwd: string, options?: WorktreeToolOptions): AgentTool {
  return {
    name: 'EnterWorktree',
    label: 'EnterWorktree',
    description:
      'Create an isolated git worktree and switch the session to it, so your changes stay on a separate branch. Takes effect on your NEXT turn (the current turn keeps the existing working directory). Use when you want to work on a change in isolation before merging.',
    parameters: agentToolParameters({
      type: 'object',
      properties: {
        name: {
          type: 'string',
          description: 'Short name for the worktree/branch (e.g. "refactor-auth")',
        },
      },
      required: ['name'],
    }),
    execute: async (toolCallId: string, params: unknown) => {
      ...
      if (!db || !sessionId)
        return errorResult('missing_session_context', 'EnterWorktree requires session context');
      ...
      if (isUnderSessionWorktrees(cwd)) {
        return errorResult(
          'already_in_worktree',
          'This session is already in a worktree. Use ExitWorktree before entering another.'
        );
      }
      if (!isGitRepository(cwd)) {
        return errorResult(
          'not_a_git_repo',
          `Cannot create a worktree: ${cwd} is not a git repository.`
        );
      }
      try {
...
export function createExitWorktreeTool(cwd: string, options?: WorktreeToolOptions): AgentTool {
  return {
    name: 'ExitWorktree',
    label: 'ExitWorktree',
    description:
      'Leave the current git worktree and switch the session back to the project root (takes effect next turn). action:"remove" also deletes the worktree if it has no uncommitted changes or unique commits; action:"keep" (default) leaves it in place for later.',
    parameters: agentToolParameters({
      type: 'object',
      properties: {
    ...
    execute: async (toolCallId: string, params: unknown) => {
      ...
      const sessionId = options?.sessionId;
      if (!db || !sessionId)
        return errorResult('missing_session_context', 'ExitWorktree requires session context');
      if (!isUnderSessionWorktrees(cwd)) {
      ...
      if (!rootPath) {
        return errorResult(
          'main_worktree_not_found',
          'Could not locate the main worktree to return to.'
        );
      }
