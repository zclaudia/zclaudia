...
import {
  buildHashlineEntries,
  formatHashlineOutput,
  hashlineSnapshotId,
  hashlineTag,
} from './hashline.js';
...
const SUMMARY_MIN_SAVINGS = 0.3; // fold >= 30% of lines, else read verbatim
...
function appendTextNotice(result: ReadToolResult, notice: string): ReadToolResult {
...
function capLinesByTokens(
  lines: string[],
  maxTokens: number
): { lines: string[]; cappedByTokens: boolean } {
...
function capTextByTokens(
  text: string,
  maxTokens: number
): { text: string; cappedByTokens: boolean } {
  const charBudget = maxTokens * 4;
...
function parsePositiveInteger(
  ...
  if (!Number.isFinite(parsed) || !Number.isInteger(parsed) || parsed < 1) {
    return {
      ok: false,
      code,
      message: `${code.replace('invalid_', '')} must be a positive integer`,
    };
  }
...
): { ok: true; offset: number; limit: number } | { ok: false; code: string; message: string } {
...
function renderTextWindow(
  ...
  return {
    text:
      formatNumberedLines(capped, offset) +
      buildReadFooter(offset, capped.length, totalLines, cappedByTokens),
...
function renderHashlineWindow(
  ...
  text: string;
  entries: ReturnType<typeof buildHashlineEntries>;
  returnedLines: number;
...
function parseRanges(
  value: unknown
): { ok: true; ranges: LineRange[] } | { ok: false; code: string; message: string } {
  if (typeof value !== 'string' || !value.trim()) {
    return {
      ok: false,
      code: 'invalid_ranges',
      message: 'ranges must be a non-empty string like "5-16,960-973"',
    };
  }
  const tokens = value
    .split(',')
    ...
    if ((m = token.match(/^(\d+)-(\d+)$/))) {
      ...
      if (start < 1 || end < start) {
        return {
          ok: false,
          code: 'invalid_ranges',
          message: `invalid range "${token}" (expected 1-based start <= end)`,
        };
      }
      ...
      if (start < 1 || count < 1) {
        return {
          ok: false,
          code: 'invalid_ranges',
          message: `invalid range "${token}" (expected 1-based start and positive count)`,
        };
      }
      ...
      if (start < 1) {
        return {
          ok: false,
          code: 'invalid_ranges',
          message: `invalid line "${token}" (lines are 1-based)`,
        };
      }
      ...
      return {
        ok: false,
        code: 'invalid_ranges',
        message: `unrecognized range "${token}" (use a-b, a+n, or a)`,
      };
    }
  }
...
export function createReadBridgeTool(cwd: string, options?: ReadToolOptions): AgentTool {
  ...
  const lastEmitted = new Map<
    string,
    { mtimeMs: number; size: number; signature: string; totalLines: number }
  >();
    ...
    parameters: agentToolParameters({
      ...
      properties: {
        ...
        ranges: {
          type: 'string',
          description:
            'For text files: read several line ranges in one call, e.g. "5-16,960-973" (also accepts a+n and bare a). Overrides offset/limit.',
        },
        pages: {
          type: 'string',
          description: 'For PDFs: page range like "1-5" or "2,7" (max 20 pages per call)',
        },
        hashline: {
          type: 'boolean',
          description: 'For text files: return content-addressed line hashes for precise edits',
        },
        full: {
          type: 'boolean',
          description: 'Read the entire file verbatim instead of a structural summary',
        },
      },
      anyOf: [{ required: ['path'] }, { required: ['file_path'] }],
    }),
    execute: async (toolCallId: string, params: unknown): Promise<ReadToolResult> => {
      const result: ReadToolResult = await (async () => {
        ...
        if (isBlockedDevicePath(filePath)) {
          return errorResult(
            'blocked_device',
            `Refusing to read special device file: ${requestedPath}`,
            {
        ...
        try {
          ...
          if (!fileStat.isFile()) {
            return errorResult('not_a_file', `Path is not a file: ${requestedPath}`, {
              path: String(requestedPath),
            });
          }
          ...
          if (imageMime) {
            ...
            if (fileStat.size > MAX_IMAGE_BYTES) {
              ...
              if (!compressed) {
                return textResult(
                  `Image file ${relPath} (${imageMime}, ${fileStat.size} bytes) exceeds the 5MB vision limit and could not be downscaled.`,
                  {
                    ok: false,
                    path: relPath,
                    size: fileStat.size,
            ...
            return {
              content: [
                { type: 'image' as const, data: buffer.toString('base64'), mimeType: imageMime },
              ],
          ...
          if (fileExt === '.pdf') {
            ...
            const capped = capTextByTokens(
              pdf.text || '(no extractable text)',
              MAX_READ_OUTPUT_TOKENS
            );
          ...
          if (fileExt === '.ipynb') {
            if (fileStat.size > MAX_NOTEBOOK_BYTES) {
              return errorResult(
                'file_too_large',
                `Notebook is too large to read: ${requestedPath}`,
                {
            ...
            if (!windowArgs.ok)
              return errorResult(windowArgs.code, windowArgs.message, { path: relPath });
          ...
          if (args.ranges !== undefined) {
            ...
            if (!parsedRanges.ok)
              return errorResult(parsedRanges.code, parsedRanges.message, { path: relPath });
            if (fileStat.size > READ_FAST_PATH_BYTES) {
              return errorResult(
                'ranges_unsupported_large',
          ...
          if (fileStat.size <= READ_FAST_PATH_BYTES) {
            ...
            try {
              ...
              if (summaryEligible) {
                ...
                if (provider) {
                  ...
                  const foldedLines = folds.reduce(
                    (sum, f) => sum + (f.endLine - f.startLine + 1),
                    0
                  );
            ...
            const view = renderTextWindow(selected, offset, totalLines);
            const hashlineView = isHashline
              ? renderHashlineWindow(relPath, text, selected, offset, totalLines)
              : null;
            const returnedLines = hashlineView?.returnedLines ?? view.returnedLines;
            ...
            recordEmitted(totalLines);
            return textResult(hashlineView?.text ?? view.text, {
              ok: true,
              path: relPath,
              offset,
              limit,
              ...
              ...(!isHashline && view.columnTruncated
                ? { columnTruncated: READ_MAX_LINE_COLUMNS }
                : {}),
              ...(hashlineView
                ? {
                    hashline: {
      ...
      const details = (result as { details?: { ok?: boolean; path?: unknown; deduped?: boolean } })
        .details;
      if (
        details &&
        details.ok !== false &&
