...
describe('translateToolEvent', () => {
    ...
    const out = translateToolEvent(
      {
        ...
        result: {
          ...
          details: { ok: true, wasActive: true },
        },
        isError: false,
      } as any,
      ctx
  ...
  it('returns undefined for tool_execution_start (covered by tool_use)', () => {
    expect(
      translateToolEvent(
        { type: 'tool_execution_start', toolCallId: 't1', toolName: 'read', args: {} } as any,
        ctx
      )
    ).toBeUndefined();
  });
  ...
  it('returns undefined for turn_start / turn_end / message_start', () => {
    ...
    expect(
      translateToolEvent(
        { type: 'turn_end', message: { role: 'assistant', content: [] }, toolResults: [] } as any,
        ctx
      )
    ).toBeUndefined();
    expect(
      translateToolEvent(
        { type: 'message_start', message: { role: 'assistant', content: [], timestamp: 0 } } as any,
        ctx
      )
    ).toBeUndefined();
  });
