...
describe('AstGrep', () => {
  ...
  it('scopes the search to a subpath', async () => {
    const dir = makeWorkspace();
    const res = await getTools(dir).AstGrep.execute('g1', { pattern: 'console.log($ARG)', path: 'sub' });
    rmSync(dir, { recursive: true, force: true });
...
describe('substituteMetaVariables', () => {
  it('substitutes single and multi metavariables', () => {
    const fake = {
      ...
      getMultipleMatches: (name: string) => (name === 'ARGS' ? [{ text: () => 'a' }, { text: () => 'b' }] : []),
    };
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
describe('AstEdit', () => {
  ...
  it('refuses to edit auto-generated files', async () => {
    const dir = makeWorkspace();
    writeFileSync(path.join(dir, 'gen.ts'), '// Code generated by protoc-gen-ts. DO NOT EDIT.\nconsole.log("gen");\n');
    const res = await getTools(dir).AstEdit.execute('e1', {
      pattern: 'console.log($ARG)',
      rewrite: 'logger.info($ARG)',
      path: 'gen.ts',
