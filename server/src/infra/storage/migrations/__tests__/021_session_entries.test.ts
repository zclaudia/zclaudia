...
describe('migration 021_session_entries', () => {
  it('creates session_entries and session_leaf with expected columns', () => {
    ...
    const entryCols = (db.prepare(`PRAGMA table_info(session_entries)`).all() as Array<{ name: string }>)
      .map((c) => c.name).sort();
    expect(entryCols).toEqual(['id', 'parent_id', 'payload', 'session_id', 'timestamp', 'type']);

    const leafCols = (db.prepare(`PRAGMA table_info(session_leaf)`).all() as Array<{ name: string }>)
      .map((c) => c.name).sort();
