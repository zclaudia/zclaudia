...
describe('030_generalize_workflow_runs', () => {
  it('adds initiator/action columns and makes workflow_id nullable', () => {
    ...
    applyMigrations(db);
    const cols = db.prepare(`PRAGMA table_info(workflow_runs)`).all() as Array<{ name: string; notnull: number }>;
    ...
    const stepCols = (db.prepare(`PRAGMA table_info(workflow_step_runs)`).all() as Array<{ name: string }>)
      .map((c) => c.name);
    ...
    const row = db.prepare(`SELECT * FROM workflow_runs WHERE id = 'r1'`).get() as Record<string, unknown>;
