...
describe('resolveAgentForSession', () => {
  ...
  beforeEach(() => {
    ...
    const globalAgent = agentRepo.create({
      name: 'Global Default', llmProfileId: llmId, model: 'm', systemPrompt: '', enabledTools: ['read'], isDefault: true,
    });
    globalDefaultAgentId = globalAgent.id;
    const projAgent = agentRepo.create({
      name: 'Project Default', llmProfileId: llmId, model: 'm', systemPrompt: '', enabledTools: ['read'],
    });
    projectDefaultAgentId = projAgent.id;
    const explicit = agentRepo.create({
      name: 'Explicit', llmProfileId: llmId, model: 'm', systemPrompt: '', enabledTools: ['read'],
    });
    explicitAgentId = explicit.id;

    const projWithDefault = projectRepo.create({
      name: 'p1', type: 'code', defaultAgentProfileId: projectDefaultAgentId, sortOrder: 0,
    });
  ...
  it('uses explicitAgentId when provided', () => {
    const { agent, llm } = resolveAgentForSession(db, { explicitAgentId, projectId: projectWithDefault });
  ...
  it('logs warn and falls back to default when explicitAgentId is stale', () => {
    ...
    const { agent } = resolveAgentForSession(db, { explicitAgentId: 'nonexistent', projectId: projectWithoutDefault });
    expect(agent.id).toBe(globalDefaultAgentId);
  ...
  it('logs warn and falls back to default LLM when agent.llmProfileId is stale', () => {
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
    const llmRepo = new LlmProfileRepository(db);
    llmRepo.create({ name: 'default-llm', providerType: 'anthropic', apiKey: 'sk', isDefault: true });
    ...
    db.prepare('UPDATE agent_profiles SET llm_profile_id = ? WHERE id = ?').run('nonexistent-llm', globalDefaultAgentId);
    db.pragma('foreign_keys = ON');
