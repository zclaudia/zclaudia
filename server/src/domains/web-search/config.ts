...
import type { UpdateWebSearchConfigRequest, WebSearchConfig, WebSearchConfigSource } from '@zclaudia/shared/core/server';
...
export function getWebSearchProviderConfig(db?: Database): WebSearchProviderConfig {
  ...
  const envBraveKey = trimToNull(process.env.ZCLAUDIA_BRAVE_SEARCH_API_KEY)
    ?? trimToNull(process.env.BRAVE_SEARCH_API_KEY);
  const storedSearxngBaseUrl = trimToNull(row?.searxng_base_url);
  const envSearxngBaseUrl = trimToNull(process.env.ZCLAUDIA_SEARXNG_BASE_URL)
    ?? trimToNull(process.env.SEARXNG_BASE_URL);

  return {
    ...(storedBraveKey || envBraveKey ? { braveApiKey: storedBraveKey ?? envBraveKey ?? undefined } : {}),
    braveApiKeySource: storedBraveKey ? 'stored' : envBraveKey ? 'env' : null,
    ...(storedSearxngBaseUrl || envSearxngBaseUrl ? { searxngBaseUrl: storedSearxngBaseUrl ?? envSearxngBaseUrl ?? undefined } : {}),
...
export function updateWebSearchConfig(db: Database, input: UpdateWebSearchConfigRequest): WebSearchConfig {
