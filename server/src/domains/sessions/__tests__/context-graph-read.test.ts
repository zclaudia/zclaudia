...
function addSession(db: Database.Database, id: string) {
  db.prepare(
    `INSERT INTO sessions (id, project_id, agent_profile_id, type, created_at, updated_at) VALUES (?, 'p','a','regular',1,1)`
  ).run(id);
}
function addEntry(
...
it('does not count toolResult entries in the collapsed edge', async () => {
  ...
  addEntry(db, 's', 'e1', null, 'message', msg('user', 'u1'));
  addEntry(db, 's', 'e2', 'e1', 'message', msg('assistant', 'a1'));
  addEntry(db, 's', 'e3', 'e2', 'message', {
    message: { role: 'toolResult', toolCallId: 't', content: [{ type: 'text', text: 'r' }] },
  });
  addEntry(db, 's', 'e4', 'e3', 'message', msg('assistant', 'a2'));
  setLeaf(db, 's', 'e4');
...
it('compaction + label nodes carry payload and targets', async () => {
  ...
  addSession(db, 's');
  addEntry(db, 's', 'e1', null, 'message', msg('user', 'u1'));
  addEntry(db, 's', 'e2', 'e1', 'compaction', {
    summary: 'sum',
    tokensBefore: 4200,
...
it('messageId jump falls back up to nearest projected message row (toolResult leaf)', async () => {
  ...
  setLeaf(db, 's', 'e3');
  db.prepare(
    `INSERT INTO messages (id, session_id, role, content, created_at, offset, tree_entry_id) VALUES ('m_a','s','assistant','a1',1,2,'e2')`
  ).run();
  db.prepare(
...
it('respects node cap and reports truncation', async () => {
  ...
  for (let i = 1; i <= 6; i++) {
    addEntry(db, 's', `e${i}`, prev, 'message', msg(i % 2 ? 'user' : 'assistant', `m${i}`));
    prev = `e${i}`;
  }
  setLeaf(db, 's', 'e6');
...
function addForkChild(
  ...
  db.prepare(
    `INSERT INTO sessions (id, project_id, agent_profile_id, type, forked_from_session_id, fork_entry_id, created_at, updated_at) VALUES (?, 'p','a','regular', ?, ?, ?, ?)`
...
it('assembles a fork family: lanes, fork edges, root/focus, non-dangling toNode', async () => {
  ...
  expect(g.sessions.find(s => s.id === 'parent')!.laneOrder).toBeLessThan(
    g.sessions.find(s => s.id === 'child')!.laneOrder
  );
  expect(g.forkEdges).toEqual([
    { fromNodeId: 'parent:e2', toSessionId: 'child', toNodeId: 'child:e2' },
  ]);
