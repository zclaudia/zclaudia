...
it('creates a new session with lineage, copied entries, projection, and leaf', async () => {
  ...
  const session = await forkSession(
    db,
    { sourceSessionId: 's', treeEntryId: 'e2' },
    {
  ...
  const msgs = db
    ...
    .all(session.id) as Array<{ content: string; t: string }>;
  expect(msgs).toEqual([
    { content: 'q1', t: 'e1' },
    { content: 'a1', t: 'e2' },
  ]);
...
it('uses a custom name when provided, falls back when blank', async () => {
  const db = setup();
  const named = await forkSession(
    db,
    { sourceSessionId: 's', treeEntryId: 'e1', name: 'My Fork' },
    {}
  );
...
it('rejects an entry not in the source session', async () => {
  const db = setup();
  await expect(
    forkSession(db, { sourceSessionId: 's', treeEntryId: 'nope' }, {})
