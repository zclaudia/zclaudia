...
it('rebuilds messages rows from the active path with offsets + tree_entry_id', async () => {
  ...
  const rows = db
    .prepare(
      `SELECT role, content, offset, tree_entry_id AS t FROM messages WHERE session_id = 's' ORDER BY offset ASC`
    )
    .all() as Array<{ role: string; content: string; offset: number; t: string }>;
...
it('clears prior messages before reprojecting (branch rewrite)', async () => {
  ...
  seedSession(db);
  db.prepare(
    `INSERT INTO messages (id, session_id, role, content, created_at, offset) VALUES ('stale','s','user','OLD',1,1)`
  ).run();
  insertEntry(db, 's', 'e1', null, { role: 'user', content: 'fresh' });

  await reproject(db, 's');

  const ids = db.prepare(`SELECT id FROM messages WHERE session_id='s'`).all() as Array<{
    id: string;
  }>;
...
it('assigns strictly increasing created_at in path order so created_at-ordered queries match offset order', async () => {
  ...
  const byOffset = db
    .prepare(
      `SELECT created_at AS c, offset AS o FROM messages WHERE session_id='s' ORDER BY offset ASC`
    )
  ...
  const displayed = (
    db
      .prepare(`SELECT offset AS o FROM messages WHERE session_id='s' ORDER BY created_at DESC`)
