...
describe('SpecChange routes', () => {
  ...
  beforeEach(() => {
    ...
    specChangeId = svc.createSpecChange({
      projectId: 'proj-1',
      subIssueId: 'i',
      slug: 'x',
      title: 'X',
    ...
    app.use(
      '/api/openspec',
      createSpecChangeRoutes({ db, specChangeService: svc, draftingService: noopDrafting })
    );
  });
  ...
  it('PUT /spec-changes/:id/delta/:capability writes delta + tracks path', async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.body.data.specChange.deltaSpecPaths).toContain(
  ...
  describe('SpecChange draft routes', () => {
    it('POST /draft-proposal returns drafted content + saves to disk', async () => {
      const draftingService = {
        draftProposal: vi
          .fn()
          .mockResolvedValue({ content: '# Drafted Proposal\n', rawResponse: '' }),
        draftDesign: vi.fn(),
        draftTasks: vi.fn(),
        draftDelta: vi.fn(),
      };
      ...
      localApp.use(
        '/api/openspec',
        createSpecChangeRoutes({
          db,
          specChangeService: svc,
          draftingService: draftingService as never,
        })
      );
      const res = await request(localApp)
        .post(`/api/openspec/spec-changes/${specChangeId}/draft-proposal`)
        .send({});
      expect(res.status).toBe(200);
      expect(res.body.data.content).toContain('Drafted Proposal');
      expect(res.body.data.specChange.status).toBe('proposing');
      const read = await request(localApp).get(
        `/api/openspec/spec-changes/${specChangeId}/proposal`
      );
    ...
    it('POST /draft-delta/:capability writes the delta + tracks path', async () => {
      const draftingService = {
        ...
        draftDelta: vi
          .fn()
          .mockResolvedValue({ content: '## ADDED Requirements\n', rawResponse: '' }),
      };
      const localApp = express();
      localApp.use(
        '/api/openspec',
      ...
      const res = await request(localApp)
        .post(`/api/openspec/spec-changes/${specChangeId}/draft-delta/auth`)
        .send({});
      expect(res.status).toBe(200);
      expect(res.body.data.specChange.deltaSpecPaths).toContain(
        'openspec/changes/x/specs/auth/spec.md'
