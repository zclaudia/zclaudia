...
export function createSpecChangeRoutes(deps: SpecChangeRoutesDeps): Router {
  ...
  router.get('/spec-changes/:id', (req: Request, res: Response) => {
    ...
    if (!sc) {
      res.status(404).json(err('NOT_FOUND', 'spec_change not found'));
      return;
    }
  ...
  const reader = (kind: 'proposal' | 'design' | 'tasks') => (req: Request, res: Response) => {
    try {
      const fn =
        kind === 'proposal'
          ? deps.specChangeService.readProposal
          : kind === 'design'
            ? deps.specChangeService.readDesign
  ...
  router.get('/spec-changes/:id/delta/:capability', (req: Request, res: Response) => {
    try {
      res
        .type('text/markdown')
        .send(deps.specChangeService.readDeltaSpec(req.params.id, req.params.capability));
    } catch (e) {
      res.status(404).json(err('NOT_FOUND', (e as Error).message));
    }
  });
  ...
  const writer = (kind: 'proposal' | 'design' | 'tasks') => (req: Request, res: Response) => {
    ...
    if (typeof content !== 'string') {
      res.status(400).json(err('VALIDATION', 'content (string) required in body'));
      return;
    }
    try {
      const fn =
        kind === 'proposal'
          ? deps.specChangeService.writeProposal
          : kind === 'design'
            ? deps.specChangeService.writeDesign
  ...
  router.put('/spec-changes/:id/delta/:capability', (req: Request, res: Response) => {
    ...
    try {
      res.json(
        ok({
          specChange: deps.specChangeService.writeDeltaSpec(
            req.params.id,
  ...
  const draftHandler =
    (kind: 'proposal' | 'design' | 'tasks') => async (req: Request, res: Response) => {
      try {
        const draft =
          kind === 'proposal'
            ? await deps.draftingService.draftProposal(req.params.id)
            : kind === 'design'
              ? await deps.draftingService.draftDesign(req.params.id)
  ...
  router.post('/spec-changes/:id/draft-delta/:capability', async (req: Request, res: Response) => {
    try {
      ...
      const specChange = deps.specChangeService.writeDeltaSpec(
        req.params.id,
        req.params.capability,
        draft.content
      );
