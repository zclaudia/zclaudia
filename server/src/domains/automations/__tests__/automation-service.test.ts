...
describe('AutomationService', () => {
  ...
  it('runs a workflow action against the persisted workflow definition', async () => {
    const wf: Workflow = {
      ...
      status: 'active',
      definition: {
        nodes: [{ id: 'n', name: 'n', type: 'shell', config: {}, position: { x: 0, y: 0 } }],
        edges: [],
        entryNodeId: 'n',
      },
      createdAt: 1,
  ...
  it('throws when a workflow action references a missing workflow', async () => {
    const svc = new AutomationService(
      ...
      () => {},
      fakeEngine() as any,
      fakeWorkflowLookup() as any
    );
    const a = svc.createAutomation({
      name: 'x',
      trigger: { type: 'manual' },
      action: { kind: 'workflow', ref: 'missing' },
    });
    await expect(
      svc.runAction(a.id, { initiator: 'manual', triggerSource: 'manual' })
...
describe('AutomationService scheduling + events', () => {
  ...
  it('fires a due interval automation on tick and computes the next run', async () => {
    ...
    (svc as any).nextRunByAutomation.forEach((_v: number, k: string) =>
      (svc as any).nextRunByAutomation.set(k, Date.now() - 1000)
    );
    await svc.tick();
    expect(engine.startRun).toHaveBeenCalledTimes(1);
  ...
  it('fires an event automation when its event is emitted', async () => {
    ...
    svc.createAutomation({
      name: 'evt',
      trigger: { type: 'event', event: 'demo.fired' },
      action: { kind: 'activity', ref: 'shell' },
    });
    svc.initialize();
...
describe('AutomationService.ensureSystemAutomations', () => {
  it('creates an immutable system permission automation bound to the system workflow', () => {
    ...
    expect(a.action).toEqual(
      expect.objectContaining({ kind: 'workflow', ref: 'system-workflow-id' })
    );
    svc.ensureSystemAutomations('system-workflow-id');
    expect(
      (svc as any).repo
        .findAll()
        .filter((x: any) => x.systemKey === SYSTEM_PERMISSION_AUTOMATION_KEY)
    ).toHaveLength(1);
  });
});
