...
describe('workflow templates', () => {
  it('does not auto-approve ExitPlanMode in the default permission workflow', () => {
    const template = BUILTIN_WORKFLOW_TEMPLATES.find(
      item => item.id === PERMISSION_WORKFLOW_TEMPLATE_ID
    );
    ...
    expect(template?.definition.nodes.some(node => node.id === 'decide_approve_exit_plan')).toBe(
      false
    );
    expect(
      template?.definition.edges.some(
        edge => edge.source === 'check_escalate' && edge.type === 'condition_true'
      )
    ).toBe(false);
  });
