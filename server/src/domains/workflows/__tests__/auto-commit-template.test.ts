...
import { BUILTIN_WORKFLOW_TEMPLATES, AI_AUTO_COMMIT_TEMPLATE_ID, SYSTEM_AI_AUTO_COMMIT_KEY } from '../templates.js';
...
describe('AI auto-commit template', () => {
  ...
  it('guards on staged changes and feeds the generated message into the commit', () => {
    ...
    expect(commit.config).toMatchObject({ stageAll: false, messageMode: 'explicit', message: '${generate.output.message}' });
    const edges = tpl.definition.edges;
    expect(edges.some((e) => e.source === 'check' && e.target === 'generate' && e.type === 'condition_true')).toBe(true);
