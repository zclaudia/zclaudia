...
function validateWorkflowDefinition(
  ...
  if (
    ...
    res.status(400).json({
      ...
      error: {
        ...
        message: 'Definition must have nodes, edges, and entryNodeId',
      },
    });
    return false;
  }
  ...
  if (
    hasNodes &&
    (!workflowDefinition.entryNodeId || !nodeIds.has(workflowDefinition.entryNodeId))
  ) {
    res.status(400).json({
      success: false,
...
export function createWorkflowRoutes(
  ...
  router.get('/workflows', (req: Request, res: Response) => {
    try {
      ...
      const workflows = projectId ? service.listWorkflows(projectId) : service.listAllWorkflows();
      res.json({ success: true, data: workflows });
    } catch (error) {
      res.status(500).json({
        success: false,
  ...
  router.post(
    '/projects/:projectId/workflows/from-template/:templateId',
    ...
    try {
      ...
      if (!cancelled) {
        return res.status(400).json({
          ...
          error: {
            code: 'INVALID_STATE',
            message: 'Run cannot be cancelled (not running or pending)',
          },
        });
      }
      res.json({ success: true, data: null });
  ...
  router.post(
    '/projects/:projectId/workflows/generate/refine',
      ...
      try {
        ...
        if (!generationId || !instruction) {
          return res.status(400).json({
            ...
            error: {
              code: 'VALIDATION_ERROR',
              message: 'generationId and instruction are required',
            },
          });
        }
        const result = await generatorService.refine(
          req.params.projectId,
          generationId,
          instruction
        );
    ...
    const builtinMeta: WorkflowStepTypeMeta[] = [
        ...
        configSchema: {
          ...
          properties: {
            prompt: {
              type: 'string',
              format: 'multiline',
              description: 'Prompt (supports {{event.key}} templates)',
            },
          },
          required: ['prompt'],
        },
      ...
      },
      {
        ...
        configSchema: {
          type: 'object',
          properties: {
            message: { type: 'string', format: 'multiline', description: 'Notification message' },
          },
          required: ['message'],
        },
      },
      ...
      },
      {
        type: 'wait',
        name: 'Wait / Approval',
        description: 'Wait or require approval',
        category: 'Flow Control',
        source: 'builtin',
      },
      {
        type: 'permission_classify',
        name: 'Permission Classify',
        description: 'Classify permission request',
        category: 'Permission',
        source: 'builtin',
      },
      {
        ...
        description: 'Analyze permission risk',
        category: 'Permission',
        source: 'builtin',
      },
      {
        ...
        description: 'Decide on permission request',
        category: 'Permission',
        source: 'builtin',
      },
    ];
