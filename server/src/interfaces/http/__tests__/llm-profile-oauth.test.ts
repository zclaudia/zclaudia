...
function makeApp() {
  ...
  const now = Date.now();
  db.prepare(
    `INSERT INTO llm_profiles (id, name, provider_type, is_default, created_at, updated_at) VALUES (?, ?, 'openai-codex', 0, ?, ?)`
  ).run('p1', 'codex', now, now);
...
describe('llm-profile-oauth router', () => {
  ...
  it('POST /:id/oauth/start (browser) returns sessionId and authUrl', async () => {
    ...
    const resp = await request(app)
      .post('/api/llm-profiles/p1/oauth/start')
      .send({ method: 'browser' });
