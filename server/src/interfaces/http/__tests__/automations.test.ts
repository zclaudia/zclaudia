...
function svcStub() {
  ...
  return {
    created,
    createAutomation: vi.fn((d: any) => {
      const a = { id: 'a1', enabled: true, createdAt: 1, updatedAt: 1, ...d };
      created.push(a);
      return a;
    }),
...
describe('POST /api/automations', () => {
  ...
  it('rejects missing name', async () => {
    const res = await request(appWith(svcStub()))
      .post('/api/automations')
      .send({ trigger: { type: 'manual' }, action: { kind: 'activity', ref: 'shell' } });
...
describe('POST /api/automations/:id/trigger', () => {
  it('runs the action manually', async () => {
    ...
    expect(svc.runAction).toHaveBeenCalledWith(
      'a1',
      expect.objectContaining({ initiator: 'automation:a1', triggerSource: 'manual' })
    );
  });
});
...
describe('PATCH /api/automations/:id', () => {
  it('whitelists patch fields, dropping isSystem/systemKey', async () => {
    ...
    const res = await request(appWith(svc))
      ...
      .send({ isSystem: true, systemKey: 'x', name: 'renamed' });
    expect(res.status).toBe(200);
    expect(svc.updateAutomation).toHaveBeenCalledWith(
      'a1',
      expect.objectContaining({ name: 'renamed' })
    );
    expect(svc.updateAutomation).toHaveBeenCalledWith(
      'a1',
      expect.not.objectContaining({ isSystem: expect.anything() })
    );
    expect(svc.updateAutomation).toHaveBeenCalledWith(
      'a1',
