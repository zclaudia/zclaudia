...
describe('shouldRegenerateTitle', () => {
  it('generates on the first user message', () => {
    expect(
      shouldRegenerateTitle({ autoTitle: undefined, autoTitleMsgCount: undefined, userMsgCount: 1 })
    ).toBe(true);
  });
  it('does not generate again until the threshold delta is reached', () => {
    expect(shouldRegenerateTitle({ autoTitle: 'X', autoTitleMsgCount: 1, userMsgCount: 2 })).toBe(
      false
    );
    expect(
      shouldRegenerateTitle({
        autoTitle: 'X',
        autoTitleMsgCount: 1,
        userMsgCount: 1 + TITLE_REGEN_THRESHOLD,
      })
    ).toBe(true);
  });
  it('never generates with zero user messages', () => {
    expect(
      shouldRegenerateTitle({ autoTitle: undefined, autoTitleMsgCount: undefined, userMsgCount: 0 })
    ).toBe(false);
  });
});
...
describe('extractTitle', () => {
  it('joins text content, strips quotes, collapses whitespace, truncates to 40', () => {
    expect(extractTitle([{ type: 'text', text: '  "Hello   world"  ' }] as any)).toBe(
      'Hello world'
    );
