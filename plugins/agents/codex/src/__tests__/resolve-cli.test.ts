...
describe('resolveCodexCli', () => {
  ...
  it('resolves from the effective provider PATH instead of the host PATH', () => {
    expect(
      resolveCodexCli(undefined, '/provider/bin', {
        platform: 'linux',
        exists: candidate => candidate === '/provider/bin/codex',
      })
    ).toBe('/provider/bin/codex');
  });
});
