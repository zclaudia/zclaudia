...
export class CodexAppServerClient {
  ...
  constructor(
    ...
    this.cliPath = resolveCodexCli(cliPath, env.PATH) || 'codex';
    this.env = env;
  ...
  private async startProcess(): Promise<void> {
    ...
    // CLI overrides include per-session bridge credentials. Never log argv.
    debugLog(`[Codex AppServer] Spawning app-server: ${this.cliPath}`);
      ...
      [...this.children].map(
        child =>
          new Promise<void>(resolve => {
            ...
            const closeTimer = setTimeout(finish, 1500);
            function finish() {
