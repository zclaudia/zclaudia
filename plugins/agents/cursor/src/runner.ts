...
const allProcesses = new Set<ReturnType<typeof spawn>>();
...
export async function* runCursor(
  ...
  proc.once('close', () => allProcesses.delete(proc));
  ...
  if (options.abortController?.signal.aborted) handleAbort();
...
export async function destroyAllCursorProcesses(): Promise<void> {
  await Promise.all([...allProcesses].map(terminateCursorProcess));
