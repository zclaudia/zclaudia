...
import {
  resolveLlmProfileStatus,
  resolveMcpServerStatus,
  resolveSkillStatus,
  resolveAgentProfileStatus,
} from './record-status-resolvers.js';
...
describe('resolveLlmProfileStatus', () => {
  ...
  it('draft when it has no model (regardless of credential)', () => {
    expect(recordChip(resolveLlmProfileStatus({ hasModel: false, hasCredential: true }))).toBe(
      'draft'
    );
    expect(recordChip(resolveLlmProfileStatus({ hasModel: false, hasCredential: false }))).toBe(
      'draft'
    );
  });
...
describe('resolveMcpServerStatus', () => {
  it('ready + usable + enabled with an endpoint and a live connection', () => {
    const s = resolveMcpServerStatus({
      hasEndpoint: true,
      enabled: true,
  ...
  it('disabled when not enabled (disabled outranks ready but not unavailable)', () => {
    expect(
      recordChip(
        resolveMcpServerStatus({ hasEndpoint: true, enabled: false, connectionState: 'connected' })
      )
    ).toBe('disabled');
  });
});
...
describe('resolveSkillStatus', () => {
  ...
  it('draft when content is not meaningful', () => {
    expect(recordChip(resolveSkillStatus({ contentMeaningful: false, eligible: true }))).toBe(
      'draft'
    );
  });
...
describe('resolveAgentProfileStatus', () => {
  it('always ready + usable when the runtime does not require an LLM', () => {
    const s = resolveAgentProfileStatus({
      requiresLlm: false,
      hasLlmBinding: false,
      hasModel: false,
      llmUsable: false,
  ...
  it('unavailable(no_llm_profile) when complete-looking but no binding, or llm_unavailable when the bound LLM is broken', () => {
    ...
    const noBind = resolveAgentProfileStatus({
      requiresLlm: true,
      hasLlmBinding: false,
      hasModel: true,
      llmUsable: false,
    ...
    const broken = resolveAgentProfileStatus({
      requiresLlm: true,
      hasLlmBinding: true,
      hasModel: true,
      llmUsable: false,
