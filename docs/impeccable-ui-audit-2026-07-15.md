...
- [ ] **P2-1 Streaming re-parse + re-highlight per token** — `features/chat/MessageList.tsx:1125-1275` (`AssistantContent`, memo keyed on `content`) + `:508-598` (`CodeBlock`→`SyntaxHighlighter`). Each delta re-runs ReactMarkdown+remark-gfm over the whole message and re-tokenizes every code block with Prism synchronously → dropped frames while streaming (worst on Android). No throttle anywhere in the path. Fix: batch/throttle with rAF (~30–60ms) or `startTransition`; memo `CodeBlock` on the code string; isolate the trailing streaming text. _Cmd: /impeccable optimize_
- [ ] **P2-2 Virtualized MessageList thrashes ResizeObservers** — `MessageList.tsx:492` (inline `ref={el => setMeasuredRef(...)}`) + `:294-300` (`setItemHeights(new Map(...))`). Past `VIRTUALIZE_THRESHOLD` (80), new closure each render tears down/re-attaches every observer; each height change re-renders the whole list. Streaming last item loops this per frame. Fix: stable memoized per-index ref factory; batch height updates via rAF. _Cmd: /impeccable optimize_
- [ ] **P2-3 `animate-loading-bar` animates `width`+`margin-left`** — `styles/index.css:449-462` → `LoadingIndicator.tsx:210`. Infinite layout+paint on main thread while a run is active (the busy window). Fix: `transform: translateX()`+`scaleX()`. _Cmd: /impeccable optimize_
...
- [ ] **P2-7 Local-PR status map raw palette** — `features/local-pr/components/LocalPRCard.tsx:28-83`. `text-yellow-500`/`cyan-500`/`gray-400`/`blue-500` — same light-mode AA failures, inconsistent with `text-destructive` this same file already uses (:564). Fix: token map. _Cmd: /impeccable colorize_
- [ ] **P2-8 WorkflowEdge mixes tokens + hardcoded hex** — `features/workflows/components/edges/WorkflowEdge.tsx:15-24`. `success` uses `hsl(var(--muted-foreground)/.4)` but error/condition/loop hardcode `#ef4444`/`#22c55e`/`#f59e0b`/`#d97706`/`#fb7185`/`#e11d48`; labels never adapt, amber low-contrast on dark canvas. Fix: `hsl(var(--destructive/success/warning))`. _Cmd: /impeccable colorize_
