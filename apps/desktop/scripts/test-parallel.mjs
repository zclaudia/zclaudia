...
const TEST_GROUPS = [
  ...
  { name: 'hooks-transport', dir: 'src/hooks/transport/__tests__', concurrent: true },
  {
    ...
    dirs: ['src/components/__tests__', 'src/components/ui/__tests__'],
    concurrent: true,
  },
  {
    name: 'components-chat',
    dirs: ['src/components/chat/__tests__', 'src/components/chat'],
    pattern: '*.test.tsx',
    concurrent: true,
  },
  ...
  {
    name: 'components-workflows',
    dirs: [
      'src/components/workflows/__tests__',
      'src/components/workflows/edges/__tests__',
      'src/components/workflows/nodes/__tests__',
    ],
    concurrent: true,
  },
  { name: 'components-agent', dir: 'src/components/agent/__tests__', concurrent: true },
  {
    name: 'components-scheduled-tasks',
    dir: 'src/components/scheduled-tasks/__tests__',
    concurrent: true,
  },
...
function runTestGroup(group) {
  return new Promise(resolve => {
    ...
    // 构建命令
    const args = ['vitest', 'run', ...testPaths, '--reporter=verbose'];
    ...
    child.on('close', async code => {
      ...
      if (code === 0) {
        console.log(
          `${colors.green}✓ ${group.name}: ${passed} passed (${duration}s)${colors.reset}`
        );
      } else {
        console.log(
          `${colors.red}✗ ${group.name}: ${failed} failed, ${passed} passed (${duration}s)${colors.reset}`
        );
      }
...
async function printSummary(results, totalTime) {
  ...
  for (const result of results) {
    ...
    if (result.status === 'FAILED') {
      ...
      console.log(
        `${colors.red}✗ ${result.name}: ${result.failed} failed${colors.reset} (${result.duration}s)`
      );
    } else {
      console.log(
        `${colors.green}✓ ${result.name}: ${result.passed} passed${colors.reset} (${result.duration}s)`
      );
    }
  }
  ...
  console.log(`\n${colors.blue}----------------------------------------${colors.reset}`);
  console.log(
    `总计: ${colors.green}${totalPassed} passed${colors.reset}, ${colors.red}${totalFailed} failed${colors.reset}`
  );
    ...
    for (const group of failedGroups) {
      ...
      try {
        ...
        const errorLines = log
          .split('\n')
          .filter(
            line => line.includes('FAIL') || line.includes('Error') || line.includes('expected')
          )
          .slice(0, 10);
