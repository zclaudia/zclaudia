...
function makeGoal(over: Partial<Goal> = {}): Goal {
  return {
    ...
    tokenBudget: GOAL_DEFAULTS.tokenBudget, tokensUsed: 0,
    maxTurns: GOAL_DEFAULTS.maxTurns, turnsUsed: 0,
...
describe('activateGoal', () => {
  ...
  it('clears an existing active goal before setting a new one', async () => {
    useGoalStore.setState({
      bySession: { s1: { goal: makeGoal({ id: 'old' }), budgetTokensUsed: 0, budgetTurnsUsed: 0, lastVerdictKind: null, lastVerdictReason: null } },
    });
  ...
  it('clears an existing paused goal before setting a new one', async () => {
    useGoalStore.setState({
      bySession: { s1: { goal: makeGoal({ id: 'old', status: 'paused' }), budgetTokensUsed: 0, budgetTurnsUsed: 0, lastVerdictKind: null, lastVerdictReason: null } },
    });
    vi.mocked(clearGoal).mockResolvedValue(undefined as never);
    vi.mocked(setGoal).mockResolvedValue(makeGoal({ id: 'new' }));
  ...
  it('does not clear a terminal existing goal before setting a new one', async () => {
    useGoalStore.setState({
      bySession: { s1: { goal: makeGoal({ id: 'old', status: 'completed' }), budgetTokensUsed: 0, budgetTurnsUsed: 0, lastVerdictKind: null, lastVerdictReason: null } },
    });
    vi.mocked(setGoal).mockResolvedValue(makeGoal({ id: 'new' }));
