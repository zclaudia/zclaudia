...
function Meter({ label, used, total }: { label: string; used: number; total: number }) {
  ...
  const right =
    label === 'Tokens' ? `${fmtTokens(used)} / ${fmtTokens(total)}` : `${used} / ${total}`;
  return (
...
export function GoalPinnedBar({
  ...
  const terminal =
    goal.status === 'completed' || goal.status === 'budget-limited' || goal.status === 'aborted';
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5">
        <Target
          size={16}
          className={paused ? 'text-muted-foreground shrink-0' : 'text-primary shrink-0'}
        />
        <button
          type="button"
          onClick={onOpen}
