...
export function UpdateBanner() {
  ...
  return (
    <div
      className={`flex items-center justify-between px-4 py-1.5 border-b text-sm flex-shrink-0 ${bgClass}`}
    >
      <div className="flex items-center gap-2 min-w-0">
        ...
        <span className="text-foreground truncate">
          ...
          {status === 'downloading' &&
            `Downloading update v${availableVersion}... ${downloadProgress}%`}
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        {status === 'available' && (
          <button
            ...
            className="px-3 py-0.5 text-xs font-medium text-foreground bg-muted/60 hover:bg-muted rounded-md transition-colors"
          >
            Download & Install
          </button>
        )}
