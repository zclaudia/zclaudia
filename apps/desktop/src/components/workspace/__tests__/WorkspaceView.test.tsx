...
describe('WorkspaceView', () => {
  ...
  it('renders panes as cards floating on the tray, without painting its own ground', () => {
    useRightWorkspaceStore.getState().openTool('A', 'memory', { openMode: 'shared' });
    const { container } = render(<WorkspaceView sessionId="A" />);
    const rootEl = container.firstChild as HTMLElement;
    // The recessed tray fill lives on the RightSidebar column, not here — this
    // container only opens the gutter that lets the tray show around the cards.
    expect(rootEl.className).not.toContain('bg-');
    expect(rootEl.className).toContain('p-2'); // gutter padding around cards
