...
interface ToolRuleListProps {
  ...
  /**
   * Show the rules in force without offering to change them. Used on phones,
   * where typing `Bash(git *)` patterns is miserable but knowing which rules
   * are live still matters when you are deciding whether to approve something.
   */
  readOnly?: boolean;
}

export function ToolRuleList({ rules, onChange, readOnly = false }: ToolRuleListProps) {
            ...
            <div className="space-y-1">
              ...
              {!readOnly && (
                <>
                  ...
                  />
                  {errors[action] && (
                    <p className="text-[10px] text-destructive">{errors[action]}</p>
                  )}
                </>
              )}
            </div>
