...
function makeGoal(over: Partial<Goal> = {}): Goal {
  return {
    id: 'g1', sessionId: 's1', objective: 'migrate to /goal', status: 'active',
    tokenBudget: 5_000_000, tokensUsed: 0, maxTurns: 50, turnsUsed: 0,
...
describe('GoalPinnedBar', () => {
  it('renders nothing when there is no goal', () => {
    const { container } = render(
      <GoalPinnedBar goal={null} tokensUsed={0} turnsUsed={0} onOpen={noop} onPauseResume={noop} onClear={noop} />,
    );
  ...
  it('shows objective, meters, pause and clear when active', () => {
    render(
      <GoalPinnedBar goal={makeGoal()} tokensUsed={1_200_000} turnsUsed={12} onOpen={noop} onPauseResume={noop} onClear={noop} />,
    );
  ...
  it('shows a resume button when paused', () => {
    render(
      <GoalPinnedBar goal={makeGoal({ status: 'paused' })} tokensUsed={0} turnsUsed={0} onOpen={noop} onPauseResume={noop} onClear={noop} />,
    );
  ...
  it('shows a dismiss button and no meters in a terminal state', () => {
    render(
      <GoalPinnedBar goal={makeGoal({ status: 'budget-limited' })} tokensUsed={0} turnsUsed={50} onOpen={noop} onPauseResume={noop} onClear={noop} />,
    );
  ...
  it('opens the dialog when the objective is clicked', () => {
    const onOpen = vi.fn();
    render(
      <GoalPinnedBar goal={makeGoal()} tokensUsed={0} turnsUsed={0} onOpen={onOpen} onPauseResume={noop} onClear={noop} />,
    );
