...
describe('promptRequestStore', () => {
  ...
  it('removes a route by id', () => {
    ...
    expect(usePromptRequestStore.getState().pendingRequests.map(item => item.requestId)).toEqual([
      'req-2',
    ]);
  });
  ...
  it('clears stale routes for a server', () => {
    usePromptRequestStore
      .getState()
      .setPendingRequest(createRoute({ requestId: 'req-1', serverId: 'gw:backend-1' }));
    usePromptRequestStore
      .getState()
      .setPendingRequest(createRoute({ requestId: 'req-2', serverId: 'gw:backend-2' }));
  ...
  it('clears routes by session', () => {
    usePromptRequestStore
      .getState()
      .setPendingRequest(createRoute({ requestId: 'req-1', sessionId: 'session-1' }));
    usePromptRequestStore
      .getState()
      .setPendingRequest(createRoute({ requestId: 'req-2', sessionId: 'session-2' }));
