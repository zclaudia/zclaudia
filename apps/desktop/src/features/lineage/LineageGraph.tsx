...
function NodeShape({ ln }: { ln: LayoutNode }) {
  ...
  return (
    <g opacity={op}>
      {ring && (
        <circle cx={ln.x} cy={ln.y} r={9} fill="none" stroke="currentColor" strokeOpacity={0.5} />
      )}
      {shape === 'square' && (
        <rect
          x={ln.x - 6}
      ...
      {shape === 'tag' && (
        <path
          d={`M${ln.x - 6} ${ln.y - 6} h10 l5 6 -5 6 h-10 z`}
          fill={fill}
          stroke="currentColor"
        />
      )}
      ...
      {shape === 'branch' && (
        <g>
          <circle cx={ln.x} cy={ln.y} r={6} fill="currentColor" />
          <circle cx={ln.x} cy={ln.y} r={2} fill="hsl(var(--background))" />
        </g>
      )}
    ...
    >
      ...
      {model.nodes.map(ln => {
        ...
        return (
          <g
            key={ln.nodeId}
            data-testid={`lineage-node-${ln.nodeId}`}
