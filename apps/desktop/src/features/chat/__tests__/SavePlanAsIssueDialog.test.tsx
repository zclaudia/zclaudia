...
describe('SavePlanAsIssueDialog', () => {
  it('pre-fills the title input with the supplied default', () => {
    render(
      <SavePlanAsIssueDialog defaultTitle="Refactor auth" onSave={vi.fn()} onCancel={vi.fn()} />
    );
  ...
  it('disables save when title is blank', () => {
    const onSave = vi.fn();
    render(<SavePlanAsIssueDialog defaultTitle="  " onSave={onSave} onCancel={vi.fn()} />);
  ...
  it('cancel invokes onCancel without saving', () => {
    ...
    const onSave = vi.fn();
    render(<SavePlanAsIssueDialog defaultTitle="x" onSave={onSave} onCancel={onCancel} />);
  ...
  it('shows a disabled spinner state when submitting=true', () => {
    render(
      <SavePlanAsIssueDialog defaultTitle="x" submitting onSave={vi.fn()} onCancel={vi.fn()} />
    );
