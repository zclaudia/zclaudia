...
const sys = { id: 'wf-sys', name: 'System Flow', isSystem: true, projectId: 'p1', definition: { nodes: [], edges: [], entryNodeId: '', triggers: [{ type: 'manual' }] } };
...
describe('AutomationWorkflowDetail', () => {
  ...
  it('renders a read-only editor for a selected system workflow', async () => {
    useTopLevelViewStore.setState({ selectedAutomationItemId: 'wf-sys', automationListRefreshNonce: 0 });
    render(<AutomationWorkflowDetail api={apiReturning(sys) as never} {...baseProps} />);
