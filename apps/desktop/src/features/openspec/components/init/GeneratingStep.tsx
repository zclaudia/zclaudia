...
export function GeneratingStep({ scan }: Props): React.ReactElement {
  ...
  const done = selected.filter(c =>
    ['generated', 'failed', 'approved', 'rejected'].includes(c.phase)
  ).length;

  return (
    <div className="space-y-3">
      <div className="text-sm">
        ⚙️ Generating specs… ({done} of {selected.length})
      </div>
      <div className="space-y-2">
        {selected.map(c => (
          <CapabilityProgressItem key={c.id} candidate={c} />
        ))}
      </div>
    </div>
  );
