...
export const useGitStore = create<GitState>(set => ({
  ...
  setStatus: (projectId, worktreePath, status) =>
    set(s => ({
      statusByPath: { ...s.statusByPath, [cacheKey(projectId, worktreePath)]: status },
    })),

  setBranches: (projectId, worktreePath, branches) =>
    set(s => ({
      branchesByPath: { ...s.branchesByPath, [cacheKey(projectId, worktreePath)]: branches },
    })),
...
export function selectLog(projectId: string, worktreePath: string) {
  return (s: GitState): GitCommit[] | undefined => s.logByPath[cacheKey(projectId, worktreePath)];
}

export function selectStash(projectId: string, worktreePath: string) {
  return (s: GitState): GitStash[] | undefined => s.stashByPath[cacheKey(projectId, worktreePath)];
}
