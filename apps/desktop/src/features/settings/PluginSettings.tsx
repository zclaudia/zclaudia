...
interface PluginSettingsProps {
  ...
  /** Show the built-in panels management section. The Plugins mode's `builtin`
   *  sub-tab renders that section separately, so it passes false there. */
  showBuiltin?: boolean;
}

export function PluginSettings({ onOpenPluginSettings, showBuiltin = true }: PluginSettingsProps) {
  const { plugins, isLoading, error, removePlugin, setError } = usePluginStore();
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Built-in Plugins */}
      {showBuiltin && <BuiltinPanelsSection />}
