...
import { selectAgentsBackends } from '../selectAgentsBackends';
...
describe('selectAgentsBackends', () => {
  it('keeps offline backends and carries their online flag', () => {
    const result = selectAgentsBackends(
      [
        b({ backendId: 'a', name: 'A', online: true }),
        b({ backendId: 'c', name: 'C', online: false }),
      ],
      null
    );
    expect(result).toEqual([
      { backendId: 'a', name: 'A', online: true },
      { backendId: 'c', name: 'C', online: false },
    ]);
  });
  ...
  it('is a pure function (does not mutate the input array order)', () => {
    ...
    selectAgentsBackends(input, null);
