    ...
    return (
      <div>
        <button
          ...
          onClick={() =>
            patchView(projectId, { screen: 'phase-board', selectedPhaseId: undefined })
          }
        >
          ← Back to Board
        </button>
    ...
    <div className="space-y-4 max-w-3xl">
      ...
      <div>
        ...
        <div className="text-sm text-muted-foreground">
          ...
          <span className="font-mono text-foreground">{phase.status}</span> · attempt{' '}
          {phase.attempt}/{phase.maxRetries}
        </div>
      </div>
      ...
      {rec && (
        <div className="border border-purple-500/30 bg-purple-500/10 rounded-md p-3 text-sm">
          ...
          <div>
            Kind: <span className="font-mono">{rec.kind}</span>
          </div>
      ...
      <div className="space-y-1 text-sm">
        {phase.generatedWorkflowId && (
          <div>
            Generated workflow: <code className={codeClass}>{phase.generatedWorkflowId}</code>
          </div>
        )}
        {phase.generatedSubagentId && (
          <div>
            Generated subagent: <code className={codeClass}>{phase.generatedSubagentId}</code>
          </div>
        )}
        {phase.reusedFromPoolId && (
          <div>
            Reused from pool item: <code className={codeClass}>{phase.reusedFromPoolId}</code>
          </div>
        )}
        {phase.currentRunId && (
          <div>
            Current sub-workflow run: <code className={codeClass}>{phase.currentRunId}</code>
          </div>
        )}
        {phase.staleSourcePhaseId && (
          <div>
            Stale source phase: <code className={codeClass}>{phase.staleSourcePhaseId}</code>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {(phase.status === 'done' || phase.status === 'failed' || phase.status === 'stale') && (
          <>
            ...
            <button
              className="px-3 py-1.5 text-sm rounded-md bg-purple-500/15 text-purple-600 hover:bg-purple-500/25"
              onClick={() => sendEvaluateImpact(socket, { runId: run.id, phaseId: phase.phaseId })}
            >
              Evaluate Impact
            </button>
            <button
              className="px-3 py-1.5 text-sm rounded-md bg-orange-500/15 text-orange-600 hover:bg-orange-500/25"
