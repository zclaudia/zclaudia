...
describe('handleBrowserMessage', () => {
  ...
  it('ignores non-browser messages', () => {
    expect(
      handleBrowserMessage({ type: 'terminal_output', terminalId: 't', data: '' } as never)
    ).toBe(false);
  });

  it('browser_opened and browser_state update session state', () => {
    expect(handleBrowserMessage({ type: 'browser_opened', sessionId: 's1', state } as never)).toBe(
      true
    );
  ...
  it('browser_error records the message; browser_engine_status updates engine', () => {
    ...
    expect(useBrowserStore.getState().engine).toMatchObject({
      status: 'downloading',
      progress: 0.4,
    });
  });
  ...
  it('browser_network upserts by id and swaps the list on replace', () => {
    ...
    handleBrowserMessage({
      type: 'browser_network',
      sessionId: 's1',
      entries: [req('a', { status: 200 })],
    } as never);
    let net = useBrowserStore.getState().sessions['s1'].network;
    expect(net.map(e => [e.id, e.status])).toEqual([
      ['a', 200],
      ['b', undefined],
    ]);
    handleBrowserMessage({
  ...
  it('browser_element_picked prefills the composer and turns pick mode off', () => {
    ...
    const element = {
      ...
      classes: ['cta'],
      text: 'Save',
      outerHtml: '<button id="cta" class="cta">Save</button>',
