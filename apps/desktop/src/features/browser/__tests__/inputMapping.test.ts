...
describe('mapPointer', () => {
  ...
  it('maps move/up and middle/right buttons', () => {
    expect(
      mapPointer(
        { offsetX: 0, offsetY: 0, button: 0, type: 'pointermove', ...noMods },
        rect,
        viewport
      )
    ...
    expect(
      mapPointer(
        { offsetX: 0, offsetY: 0, button: 2, type: 'pointerup', ...noMods },
        rect,
        viewport
      )
    ...
    expect(
      mapPointer(
        { offsetX: 0, offsetY: 0, button: 1, type: 'pointerdown', ...noMods },
        rect,
        viewport
      )
  ...
  it('letterboxes a fixed device viewport: uniform fit scale, centering offset, margin clamp', () => {
    ...
    expect(
      mapPointer(
        { offsetX: 160, offsetY: 30, button: 0, type: 'pointerdown', ...noMods },
        box,
        device
      )
    ...
    expect(
      mapPointer(
        { offsetX: 10, offsetY: 30, button: 0, type: 'pointerdown', ...noMods },
        box,
        device
      )
    ).toMatchObject({ x: 0 });
    expect(
      mapPointer(
        { offsetX: 390, offsetY: 30, button: 0, type: 'pointerdown', ...noMods },
        box,
        device
      )
    ).toMatchObject({ x: 99 });
    ...
    expect(
      mapPointer(
        { offsetX: 100, offsetY: 150, button: 0, type: 'pointerdown', ...noMods },
  ...
  it('returns null for unknown event types', () => {
    expect(
      mapPointer(
        { offsetX: 0, offsetY: 0, button: 0, type: 'pointercancel', ...noMods },
        rect,
        viewport
      )
    ).toBeNull();
...
describe('mapKey', () => {
  it('includes text for printable keydown', () => {
    expect(mapKey({ type: 'keydown', key: 'a', code: 'KeyA', ...noMods })).toEqual({
      kind: 'key',
      type: 'down',
      key: 'a',
      code: 'KeyA',
  ...
  it('omits text for non-printable keys and maps keyup', () => {
    expect(mapKey({ type: 'keydown', key: 'ArrowLeft', code: 'ArrowLeft', ...noMods })).toEqual({
      kind: 'key',
      type: 'down',
      key: 'ArrowLeft',
      code: 'ArrowLeft',
    ...
    expect(mapKey({ type: 'keyup', key: 'a', code: 'KeyA', ...noMods })).toEqual({
      kind: 'key',
      type: 'up',
      key: 'a',
      code: 'KeyA',
  ...
  it('adds text for Enter/Backspace-like editing keys via key name (Enter → \\r)', () => {
    expect(mapKey({ type: 'keydown', key: 'Enter', code: 'Enter', ...noMods })).toEqual({
      kind: 'key',
      type: 'down',
      key: 'Enter',
      code: 'Enter',
